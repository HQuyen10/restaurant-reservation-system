...
describe('Admin Report Page', () => {
    ...
    beforeEach(() => {
        ...
        ({ formatCurrency, renderTable, loadReport } = require('../static/js/admin_report.js'));
        window.reportOnload = window.onload;
        window.onload = null;
    });
    ...
    test('window onload loads restaurants and report data', async () => {
        global.fetch
            .mockResolvedValueOnce({
                ok: true,
                json: async () => [{ RestaurantID: 1, RestaurantName: 'Golden' }]
            })
            .mockResolvedValueOnce({
                status: 200,
                json: async () => ({
                    ...
                    total_6_months: 6000,
                    restaurant_count: 1,
                    months: [{ label: '05/2026' }],
                    restaurants: []
                })
            });

        await window.reportOnload();

        expect(document.getElementById('restaurant_id').textContent).toContain('Golden');
        expect(document.getElementById('status_text').textContent).toBe('Hoàn tất');
