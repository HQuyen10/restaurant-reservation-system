...
async function loadMenu() {
    ...
    if (!token) {
        if (menuContainer) {
            menuContainer.innerHTML = `<p class="empty-message">Vui lòng đăng nhập để xem menu.</p>`;
        }
        return;
    }
    ...
    try {
        ...
        try {
            ...
            console.error("Lỗi parse menu JSON:", jsonError);
        }
        ...
        if (response.ok && Array.isArray(data) && data.length > 0) {
            ...
            if (menuContainer) {
                menuContainer.innerHTML = `<p class="empty-message">Không có món hoặc lỗi!</p>`;
            }
            console.log("Không có món hoặc lỗi!", data);
        }
