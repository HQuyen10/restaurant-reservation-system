...
def get_res_menu(restaurant_id):
    ...
    for m in menus:
        ...
        result.append({
            ...
            "category": getattr(getattr(m, "category", None), "CategoryName", ""),
...
def create_booking(data):
    try:
        ...
        return {"error": "Invalid booking date/time"}
    ...
    booking = Reservation(
        ...
        GuestCount=int(data.get("GuestCount", data.get("people", 1))),
...
def pay_order_service(table_id, payment_data=None):
    if table_id is None:
        return {"error": "Invalid table id"}, 400
    ...
    try:
        ...
        db.session.rollback()
        return {"error": "Thanh toán thất bại"}, 500
