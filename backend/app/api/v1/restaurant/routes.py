...
def get_menu_api():
    ...
    token_data = get_jwt()
    res_id = token_data.get("restaurant_id")

    if not res_id:
        user_info = User.query.get(current_user_id)
        res_id = getattr(user_info, "RestaurantID", None)

    if not res_id:
        return jsonify({"message": "Không tìm thấy nhà hàng của nhân viên"}), 400
...
def pay_order(table_id):
    result, status = pay_order_service(table_id)
    return jsonify(result), status
