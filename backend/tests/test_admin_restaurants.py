...
@pytest.mark.parametrize("status, mock_val", [
    ...
    ("Active", [{"id": 1}])   # 2. Lọc theo trạng thái
])
...
# 3-18. Các case thao tác nhà hàng
@pytest.mark.parametrize("method, url, payload, mock_ret, expected_status", [
    ("PUT", "/api/v1/admin/restaurants/1", {"RestaurantName": "Lẩu băng chuyền"}, ({"msg": "OK"}, 200), 200),                                            # 3. Cập nhật thành công
    ("PUT", "/api/v1/admin/restaurants/99", {"RestaurantName": "Chi nhánh 1"}, ({"msg": "Không tìm thấy nhà hàng"}, 404), 404),                          # 4. Cập nhật không tìm thấy
    ("PUT", "/api/v1/admin/restaurants/3", {"RestaurantName": " "}, ({"msg": "Tên nhà hàng không được để trống"}, 400), 400),                          # 5. Tên trống
    ("PUT", "/api/v1/admin/restaurants/3", {"RestaurantName": "Hotpot @ 123"}, ({"msg": "Tên nhà hàng không hợp lệ"}, 400), 400),                          # 6. Tên sai định dạng
    ("PUT", "/api/v1/admin/restaurants/3", {"Phone": ""}, ({"msg": "Số điện thoại không được để trống"}, 400), 400),                                   # 7. Số điện thoại trống
    ("PUT", "/api/v1/admin/restaurants/3", {"Phone": "123456789"}, ({"msg": "Số điện thoại phải là định dạng số, đúng 10 chữ số và bắt đầu bằng số 0"}, 400), 400), # 8. Số điện thoại sai
    ("PUT", "/api/v1/admin/restaurants/3", {"Address": ""}, ({"msg": "Địa chỉ không được để trống"}, 400), 400),                                         # 9. Địa chỉ trống
    ("PUT", "/api/v1/admin/restaurants/3", {"Address": "HCM !!!"}, ({"msg": "Khong tim thay"}, 400), 400),                                               # 10. Địa chỉ sai định dạng
    ("PUT", "/api/v1/admin/restaurants/3", {"Email": "contact_hotpot.vn"}, ({"msg": "Email không hợp lệ"}, 400), 400),                                 # 11. Email sai
    ("PUT", "/api/v1/admin/restaurants/1", {"description": "New"}, ({"msg": "OK"}, 200), 200),                                                         # 12. Cập nhật mô tả
...
# 19-33. Các case tạo mới
@pytest.mark.parametrize("url, payload, mock_ret, expected_status, use_admin", [
    ("/api/v1/restaurants/registerRestaurant", {"RestaurantName": "Nhà Hàng Mới"}, ({"id": 2}, 201), 201, False),                                                   # 19. Đăng ký
    ("/api/v1/admin/restaurants", {"RestaurantName": "Nhà Hàng Mới"}, ({"id": 1}, 201), 201, True),                                                                 # 20. Admin thêm mới
    ...
    ("/api/v1/admin/restaurants", {"RestaurantName": " Hotpot "}, ({"id": 2}, 201), 201, True),                                                                     # 32. Xử lý khoảng trắng
    ("/api/v1/admin/restaurants", {"UserID": 12, "CuisineID": 1}, ({"id": 1}, 201), 201, True),                                                                     # 33. Có khóa liên kết
])
