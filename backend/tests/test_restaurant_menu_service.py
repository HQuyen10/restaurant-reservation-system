...
# Giả lập MENU_DATA cho ảnh fallback
MOCK_MENU_DATA = [{"name": "Lau Ca", "image": "pho.jpg"}]

# 1. Kiểm tra validation create_food
...
# 7. Cập nhật hình ảnh và bỏ qua null/undefined
...
# 8. Lấy menu admin và xử lý ảnh fallback
def test_get_menu_res_admin_logic(monkeypatch, app_context):
    ...
    mock_food_items = [
        ...
        MagicMock(FoodID="F2", FoodName="Lau Ca", Price=30000, Image_URL=None, RestaurantID=1, Visible=False), # Ảnh fallback lauca.jpg
        MagicMock(FoodID="F3", FoodName="Lạ", Price=20000, Image_URL="", RestaurantID=1, Visible=True),    # Không khớp ảnh
        MagicMock(FoodID=None, FoodName="", Price=None, Image_URL=None, RestaurantID=1, Visible=True)     # Giá trị mặc định
    ]
    ...
    # Giả lập MENU_DATA trong service
    import sys
    ...
    assert res[1]["image"] == "pho.jpg"       # Lấy ảnh từ MENU_DATA
    assert res[2]["image"] == ""              # Không tìm thấy -> rỗng
    assert res[3]["id"] is None               # ID mặc định
    assert res[3]["price"] == 0.0             # Giá mặc định

# 9. Kiểm tra lỗi import ảnh
def test_get_menu_res_admin_import_exception(monkeypatch, app_context):
    # Giả lập lỗi import module image
    import sys
    ...
    # Không được crash khi import ảnh lỗi
    try:
        res = service.get_menu_res_admin(1)
